...
class LocalDevice:
    ...
    def __init__(
        ...
        if len(username) > 16:
            raise InvalidLengthError(
                "username length must be not more than 16 characters"
            )
        if len(password) > 16:
            raise InvalidLengthError(
                "password length must be not more than 16 characters"
            )
        self._name = name
    ...
    def from_encoded(cls, ampersand_string: str) -> "LocalDevice":
        ...
        try:
            ...
            print(f"Error processing {username_enc_b64}: {e}", err=True)
            username = "Unable to decode"
            print(username)
        finally:
            if e is None:
                print(f"Username Successfully Decoded: {username}")
            else:
                print(f"Encoded username string was: {username_enc_b64}")
        e = None
        pwsuccess = True
        try:
            password = HikAES().decrypt_b64_to_str(password_enc_b64).rstrip("\x00")
        except Exception as e:
            print(f"Error processing {password_enc_b64}: {e}", err=True)
            password = "Could not decode"
            print(password)
        finally:
            if e is None:
                pwsuccess = True
                # print(f"Password Successfully Decoded: {password}")
            else:
                pwsuccess = False
                # print(f"Encoded password was {password_enc_b64}")
        return cls(
